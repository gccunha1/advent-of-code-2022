...
void Solution::ParseInput(const std::vector<std::string> &input) {
    ...
    std::function<int(int)> current_operation;
    std::function<int(int)> current_test;
    ...
    auto test_template = [](int item, int test_val, int monkey1, int monkey2) -> int {
        if (item % test_val == 0) {
            return monkey1;
        }
        return monkey2;
    };
    auto operation_template = [](int item, char operator_, int operand2) {
        switch (operator_) {
        case '*':
    ...
    for (size_t i = 0; i < 20; i++) {
        for (size_t j = 0; j < monkeys.size(); j++) {
            while (monkeys.at(j).HasItems()) {
                auto item = monkeys.at(j).RemoveItem();

                item = monkeys.at(j).Operate(item) / 3;
